import{h as c,i as p,j as m}from"./storage-D3wuKbZV.js";const i=document.querySelector("#app");function o(t){return String(t).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function x(t){return t.reduce((s,e)=>(e.tomatoSize==="small"&&(s.small+=e.tomatoCount),e.tomatoSize==="large"&&(s.large+=e.tomatoCount),s),{small:0,large:0})}function d(t){const[s,e,r]=t.split("/").map(Number),l=new Date(s,e-1,r);return`${t}（${["日","一","二","三","四","五","六"][l.getDay()]}）`}function g(t){const s=Array.from({length:t.tomatoCount},()=>"<span>🍅</span>").join("");return`
    <div class="tomato-icons ${t.tomatoSize==="large"?"tomato-icons-large":"tomato-icons-small"}" aria-label="${o(t.tomatoLabel)}">
      ${s}
    </div>
  `}function b(t){const s=new Map;return t.forEach(e=>{s.has(e.date)||s.set(e.date,[]),s.get(e.date).push(e)}),Array.from(s.entries()).map(([e,r])=>({date:e,items:r}))}function n(){const t=c(),s=x(t),e=b(t);i.innerHTML=`
          ...
          ${e.length>0?e.map(l=>`
                      <section class="space-y-4">
                        <div class="date-group-header flex items-center gap-3">
                          ...
                          <h2 class="text-2xl md:text-3xl font-bold text-pink-500">${o(d(l.date))}</h2>
                        </div>
                        <div class="space-y-4">
                          ${l.items.map(a=>`
                                <article class="history-item bg-white rounded-[1.75rem] border border-pink-100 shadow-sm px-5 py-4 md:px-6 md:py-5 flex items-center justify-between gap-4 flex-wrap">
                                  <div class="flex items-center gap-4 min-w-0">
                                    ...
                                    <div class="min-w-0">
                                       ...
                                       ${a.videoName?`<p class="text-base md:text-lg text-pink-400 mt-2">影片：${o(a.videoName)}</p>`:""}
                                  ...
                                  <div class="flex items-center gap-4 ml-auto flex-wrap justify-end">
                                    <div class="text-right text-base md:text-lg text-gray-400">
                                      <p class="font-bold text-pink-400">${o(d(a.date))}</p>
  ...
  `,i.querySelectorAll("[data-delete-id]").forEach(l=>{l.addEventListener("click",()=>{const a=l.dataset.deleteId;a&&(p(a),n())})});const r=document.querySelector("#clear-history");r&&r.addEventListener("click",()=>{window.confirm("確定要清空全部紀錄嗎？這個操作無法復原。")&&(m(),n())})}n();
